...
router.get('/episodios/:id(\\d+)', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.put('/episodios/:id(\\d+)', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { error, value } = episodioSchema.validate(req.body, {
      stripUnknown: true,
...
router.delete('/episodios/:id(\\d+)', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
